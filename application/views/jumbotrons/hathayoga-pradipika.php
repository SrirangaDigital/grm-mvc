<div class="jumbotron jumbotron-fluid paper-background">
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-10">
                <img src="{{PUBLIC_URL}}images/floral-decor-top.png" alt="floral-decor" class="vertical-floral-delimiter top">
                <h4>श्री-स्वात्माराम-योगीन्द्र-विरचिता</h4>
                <h1>हठ-योग-प्रदीपिका</h1>

                <h5>ब्रह्मानन्द-कृता</h5>
                <h2>ज्योत्स्ना-व्याख्या</h2>
                <ul class="list-inline auxiliary-links">
                    <li class="list-inline-item"><a href="#HYP_C01">प्रथमोपदेशः</a></li>
                    <li class="list-inline-item"><a href="#HYP_C02">द्वितीयोपदेशः</a></li>
                    <li class="list-inline-item"><a href="#HYP_C03">तृतीयोपदेशः</a></li>
                    <li class="list-inline-item"><a href="#HYP_C04">चतुर्थोपदेशः</a></li>
                </ul>
                <p>change script to <a class="scriptSwitch" href="" data-lang-code="iast"></a> <a class="scriptSwitch" href="" data-lang-code="devanagari"></a> <a class="scriptSwitch" href="" data-lang-code="kn"> <a class="scriptSwitch" href="" data-lang-code="bn"> <a class="scriptSwitch" href="" data-lang-code="ta"></a></p>
            </div>
        </div>
    </div>
</div>
